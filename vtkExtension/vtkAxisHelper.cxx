...
vtkAxisHelper::vtkAxisHelper()
{
  ...
  renderer->AddObserver(
      vtkCommand::StartEvent, this, &vtkAxisHelper::OrientWidgetRepresentation);
}

//------------------------------------------------------------------------------
vtkRenderer *vtkAxisHelper::GetParentRenderer()
{
  return this->ParentRenderer;
}

//------------------------------------------------------------------------------
void vtkAxisHelper::SetParentRenderer(vtkRenderer *parentRen)
{
  ...
  if (parentRen != nullptr)
  {
    ...
    if (renWin != nullptr)
    {
      ...
      this->ResizeObserverTag = renWin->AddObserver(
          vtkCommand::WindowResizeEvent, this, &vtkAxisHelper::SquareResize);
    }
  }
...
//----------------------------------------------------------------------------
void vtkAxisHelper::CreateDefaultRepresentation()
{
  if (!this->WidgetRep)
  {
...
void vtkAxisHelper::ComputeWidgetState(int X, int Y, int modify /* =0*/)
{
...
//----------------------------------------------------------------------------
void vtkAxisHelper::SelectAction(vtkAbstractWidget *w)
{
  // cast to ourself
  vtkAxisHelper *const self = vtkAxisHelper::SafeDownCast(w);
  if (self == nullptr)
  {
    return;
  }
...
//----------------------------------------------------------------------------
void vtkAxisHelper::EndSelectAction(vtkAbstractWidget *w)
{
  // cast to ourself
  vtkAxisHelper *const self = vtkAxisHelper::SafeDownCast(w);
  if (self == nullptr)
...
//----------------------------------------------------------------------------
void vtkAxisHelper::MoveAction(vtkAbstractWidget *w)
{
  // cast to ourself
  vtkAxisHelper *const self = vtkAxisHelper::SafeDownCast(w);
  if (self == nullptr)
...
//-----------------------------------------------------------------------------
void vtkAxisHelper::OrientParentCamera(double back[3], double up[3])
{
  if (this->ParentRenderer == nullptr)
  {
    return;
...
//-----------------------------------------------------------------------------
void vtkAxisHelper::OrientWidgetRepresentation()
{
  if (this->ParentRenderer == nullptr)
  {
    return;
...
void vtkAxisHelper::InterpolateCamera(int t)
{
  if (this->ParentRenderer == nullptr)
  {
    return;
...
//----------------------------------------------------------------------------
void vtkAxisHelper::SquareResize()
{
...
void vtkAxisHelper::SetViewport(double xmin, double ymin, double xmax, double ymax)
{
  this->DefaultRenderer->SetViewport(xmin, ymin, xmax, ymax);
}

//----------------------------------------------------------------------------
void vtkAxisHelper::PrintSelf(ostream &os, vtkIndent indent)
{
